...
RadarPathElement::RadarPathElement(const ZCHX::Data::ITF_RadarRouteNode& data, zchxMapWidget* frame)
    :Element(data.mTopNode.centerlatitude, data.mTopNode.centerlongitude, frame, ZCHX::Data::ELE_RADAR_RADARPATH)
{
    setData(data);
}
...
//    qDebug()<<"show rect now.."<<mView->framework()->getZoom();
    ...
    if (getIsUpdate())
    {
        ...
        QPointF centerPos = mView->framework()->LatLon2Pixel(mRoutePath.mTopNode.centerlatitude, mRoutePath.mTopNode.centerlongitude).toPointF();
        ...
        textMap[pointfTostring(centerPos)].append(QString("00-%1").arg(mRoutePath.mTopNode.rectNumber));
        for(int i=0; i<mRoutePath.mPathList.size(); i++)
        {
            QPolygonF path;
            path.append(centerPos);
            ITF_RadarRectDefList children = mRoutePath.mPathList[i];
            for(int k=0; k<children.size();k++)
            {
                ...
                textMap[pointfTostring(curPos)].append(QString("%1%2-%3").arg(i+1).arg(k+1).arg(child.rectNumber));
        ...
        if(mRoutePath.mPathList.size() == 0)
        {
            //没有子节点的情况
            QPolygonF poly;
            for(int m=0; m<mRoutePath.mTopNode.predictionArea.size();m++)
            {
                ITF_SingleVideoBlock block = mRoutePath.mTopNode.predictionArea[m];
