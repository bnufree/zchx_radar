...
void AISChartElement::drawElement(QPainter *painter)
{
    if(!isDrawAvailable(painter)) return;

    PainterPair chk(painter);
    QPointF pos = getViewPos();
    //设定图形的显示区域,将长度转换成像素距离
    int lenth = qRound(ZCHX::Utils::getDistanceByPixel(mView->framework(), m_data.lat, m_data.lon, m_data.radius, 90, pos));
    QRectF rect(0, 0, 2*lenth -1, 2*lenth -1);
    rect.moveCenter(pos);
    if(rect.width() < 2) return;
//    qDebug()<<"rect:"<<rect;

    //根据数据的类型进行贴图
    if(m_data.format.compare("SVG", Qt::CaseInsensitive) == 0)
    {
        //贴图SVG
        QSvgRenderer render;
        if(render.load(m_data.imageData))
        {
    ...
    } else
    {
        //直接贴图
        QPixmap pixmap;
        if(pixmap.loadFromData(m_data.imageData, m_data.format.toUtf8().constData()))
        {
...
void AISChartElement::setData(const ZCHX::Data::ITF_AIS_Chart& data)
{
    FixElement::setData(data);
    setID(m_data.name);
    Element::setLatLon(m_data.getLat(),m_data.getLon());
...
void LocalMarkElement::drawElement(QPainter *painter)
{
    ...
    painter->setBrush(Qt::red);
    QPen  pen = QPen(QColor(Qt::GlobalColor::blue));
    painter->drawEllipse(getViewPos(), 2, 2);
