...
{
    addLayer(ZCHX::LAYER_ISLAND, ZCHX::TR_LAYER_ISLAND, true);
    //雷达回波图层
    addLayer(ZCHX::LAYER_RADARRECT, ZCHX::TR_LAYER_RADARRECT, true);
    ...
    //雷达
    addLayer(ZCHX::LAYER_RADAR_SITE, ZCHX::TR_LAYER_RADAR_SITE, true);
    ...
    //添加自定义线图层
    addLayer(ZCHX::LAYER_STATISTCLINE, ZCHX::TR_LAYER_STATISTCLINE, true);
...
//    qDebug()<<"layer:"<<layer->type()<<layer->displayName()<<" parent:"<<(parent.get() == NULL ? "" : parent->type());
    ...
    {
        std::shared_ptr<MapLayer> exist = getLayer(layer->type());
        if(exist) exist->setVisible(layer->visible());
//        qDebug() << "Layer: " << layer->type() << " had been added!"<<"visible:"<<layer->visible();
        return;
    }
...
void MapLayerMgr::addLayer(const QString& curLayer, const QString& curDisplayName, bool curVisible, const QString &parentLayer)
{
    ...
    {
        std::shared_ptr<MapLayer> exist = getLayer(curLayer);
        if(exist) exist->setVisible(curVisible);
        qDebug() << "Layer: " << curLayer << " had been added!"<<"visible:"<<curVisible;
        return;
    }
