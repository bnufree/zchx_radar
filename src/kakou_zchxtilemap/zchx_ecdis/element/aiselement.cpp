    ...
    : FixElement<ZCHX::Data::ITF_AIS>(ele, ZCHX::Data::ELE_AIS, ZCHX::LAYER_AIS, w)
{
    ...
    m_iHistoryTrackWidth = 2;
//    m_needScreenshot = false;
...
//    painter->setPen(QPen(QColor(m_sHistoryTrackStyle), m_iHistoryTrackWidth, Qt::SolidLine));
    painter->setPen(QPen(QColor(Qt::blue), 4, Qt::SolidLine));
...
void AisElement::drawElement(QPainter *painter)
{
    ...
    // 根据船舶类型设置船舶颜色
    getShipColor(m_data.cargoType, fillColor);
...
void AisElement::drawTargetInformation(QList<QPolygon>& list, int mode, QPainter *painter)
{
...
//            qDebug()<<"mid point:"<<mid_point<<pos<<angle<<label_rect;
    ...
    if (!warnName.isEmpty() && m_data.warn_status != ZCHX::Data::WARN_CHANNEL_COLLISION)
    {
        ...
        painter->drawPixmap(pos.x() - 20 - devicePix.width() / 2, pos.y() - 40 - devicePix.height() / 2,
                            devicePix.width(), devicePix.height(), devicePix);

        painter->setPen(QPen(Qt::red, 1));
        QRect rectangle = QRect(pos.x() - 120, pos.y() - 50, 300, 30);
...
{
    return (hasCamera() || isForceImage || m_data.is_fleet);
}
...
//bool AisElement::contains(const QPoint &pos) const
//{
//    ZCHX::Data::Point2D  curPos = this->framework()->LatLon2Pixel(m_data.getLat(),m_data.getLon());

//    qDebug() << "CCCCCCCCCCCCCCCCCCCCCC" << pos.x()
//             << "|" << pos.y()
//             << "|" << curPos.x
//             << "|" << curPos.y;
