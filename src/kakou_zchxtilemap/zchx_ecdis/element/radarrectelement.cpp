...
{    
    ...
    if(size > 0 )
    {
        ...
        int last_time = int(mRect.mHistoryRects.last().timeOfDay);
        int first_time = int(mRect.mHistoryRects.first().timeOfDay);
        ...
        if(early_first)
        {
            ...
            {
                ZCHX::Data::ITF_RadarRectDef temp = mRect.rects[size-1-i];
                mRect.rects[size-1-i] = mRect.rects[i];
                mRect.rects[i] = temp;
            }
        }
        //3) 检查历史轨迹的第一个点是否和当前的点重合,如果重合,就将他删除
        int  cur_time = int(mRect.mCurrentRect.timeOfDay);
        first_time = int(mRect.mHistoryRects.first().timeOfDay);
        if(cur_time == first_time) mRect.mHistoryRects.removeFirst();
        //4)从当前点开始计算轨迹的运动方向,反向点开始截断
        if(CUT_FISRT_DIRECTLY)
        {
            if(mRect.mHistoryRects.size() > 0)
            {
                ITF_RadarRectDef last = mRect.mHistoryRects.last();
                mRect.mHistoryRects.clear();
                last.angle = mRect.mCurrentRect.angle;
                mRect.mHistoryRects.append(last);
            }

        } else
        {
            //重新计算各个点的运动方向
            ITF_RadarRectDef pre_rect = mRect.current;
            for(int i=0; i<mRect.mHistoryRects.size();)
            {
            ...
            double pre_angle = mRect.mCurrentRect.angle;
            for(int i=0; i<mRect.mHistoryRects.size(); i++)
            {
                ...
                {
                    //开始截断
                    mRect.rects = mRect.mHistoryRects.mid(0, i);
                    break;
                }
        ...
        //开始进行插值处理
        mRect.mHistoryRects.prepend(mRect.current);
        for(int i= 1; i < mRect.mHistoryRects.size();)
        {
            ITF_RadarRectDef pre = mRect.rects[i-1];
            ITF_RadarRectDef cur = mRect.rects[i];
            ITF_RadarRectDefList midList;
            calculateSkipRect(midList, pre, cur);
            for(int k = 0; k < midList.size(); k++)
            {
                mRect.mHistoryRects.insert(i, midList[k]);
                i++;
            }
            i++;
        }
        mRect.mHistoryRects.removeFirst();
    }
#endif
    setIsUpdate(true);
}
...
void RadarRectGlowElement::drawPolygon(QPainter* painter, const QColor &edgeColor, const QColor &brushColor, const QPolygon &poly, double lat, double lon)
{
    painter->save();
...
//    painter->drawPolygon(poly.translated(mRect.mCurrentRect.pixWidth * (-1) * 0.5, mRect.mCurrentRect.pixHeight * (-1) * 0.5));
...
//    painter.drawText(QPoint(0, 0), QString("%1,%2").arg(lat, 0, 'f', 6).arg(lon, 0, 'f', 6));
    ...
    for(int i=0; i<mRect.mCurrentRect.blocks.size(); i++)
    {
        lat = mRect.mCurrentRect.blocks[i].latitude;
        lon = mRect.mCurrentRect.blocks[i].longitude;
...
//        painter->drawEllipse(QPointF(centerPos.x(), centerPos.y()), radius, radius);
        ...
        int hisNum = mRect.mHistoryRects.size();
        double angle = 0;
        if (hisNum > 2)
        {
                                                        ...
                                                        int continueTimeIndex)
{
    ...
    int continueTime = data.mHistoryRects.at(0).updateTime -
            data.mHistoryRects.at(data.mHistoryRects.size() - 1).updateTime;
