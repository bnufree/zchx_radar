...
public:
    ...
    void itfToolBarStatistcLineAreaAdd(); //添加自定义线
    void itfToolBarStatistcLineZONESelected(); //选择自定义线
    void itfToolBarStatistcLineZONEmove(); //整体平移自定义线
    void itfToolBarStatistcLineZONEmoveCtrlPoint();//微调整控制点
    void itfToolBarStatistcLineZONEDelCtrlPoint(); //删除锚泊控制点
    void itfToolBarStatistcLineZONEAddCtrlPoint(); //添加锚泊控制点

    void itfToolBarSelectPlayZone(); //选择回放区域
    ...
    /*!
         * \brief 通过 uuid   获取自定义线
         */
    void itfStatistcLineZoneData4id(int uuid, ZCHX::Data::tagITF_StatistcLine &info, bool &ok);
	...
	void setEcdisMapSource(ZCHX::TILE_SOURCE source);
...
signals: //定义发送信号接口
    ...
    void itfCreateStatistcLineZone(ZCHX::Data::ITF_StatistcLine);
    void itfSignalIsSelected4StatistcLineZone(const ZCHX::Data::ITF_StatistcLine &info);
    ...
    void signalIsDoubleClicked4StatistcLineZone(const ZCHX::Data::ITF_WarringZone &info);//自定义线双击
    ...
    void itfSignalPlayDirectVhfText(QString shipId);
    //图层
    /*!
...
public slots: //定义Recive数据接口
    ...
    bool itfSetSingleAisData(QString id, const QList<ZCHX::Data::ITF_AIS> &data, bool force = false);//设置历史轨迹
    ...
    void itfSetAisSite(const QList<ZCHX::Data::ITF_AisSite> &data); //设置Ais基站列表
    void itfSetAidtoNavigation(const QList<ZCHX::Data::ITF_AidtoNavigation> &data); //设置Ais助航报告
    void itfSetRadarSite(const QList<ZCHX::Data::ITF_RadarSite> &data); //设置雷达基站列表
    ...
    void itfSetStatistcLine(const QList<ZCHX::Data::ITF_StatistcLine> &data);  //设置自定义线
    void itfImportStatistcLine(const std::vector<std::pair<double, double> > &data); // 导入自定义线

    void itfSetFleet(const QList<ZCHX::Data::ITF_Fleet> &data);  //设置船队

    void itfSetNaviMark(const QList<ZCHX::Data::ITF_NaviMark> &data); //设置虚拟航标
    ...
    void itfSetRadarDisplayInfo(bool showRadarLabel, int targetSizeIndex, int traceLenIndex, int continueTimeIndex); //设置雷达显示方式
    void itfSetRadarLabelVisible(bool showRadarLabel);
...
protected Q_SLOTS:
    ...
    void OnStatistcLineAreaAdd(); //添加自定义线
    void OnStatistcLineZONESelected(); //选中当前自定义线
    void OnStatistcLineZONEMove(); //整体平移自定义线
    void OnStatistcLineZONEMoveCtrlPoint();//微调整自定义线控制点
    void OnStatistcLineZONEDelCtrlPoint(); //删除自定义线控制点
    void OnStatistcLineZONEAddCtrlPoint(); //添加自定义线控制点

    void OnSelectPlayZone(); //选择回放区域
