...
zchxMapLoadThread::zchxMapLoadThread(QObject *parent) : QThread(parent)
{
    ...
    qRegisterMetaType<MapRangeData>("const MapRangeData&");
    mImgSync = true;

    mImgUrl = Profiles::instance()->value(MAP_INDEX, IMG_URL).toString().trimmed();
    ...
    mGoogleMapPath = Profiles::instance()->value(MAP_INDEX, GOOGLE_MAP_DIR).toString().trimmed();
    if(mGoogleMapPath.right(1) == "/") mGoogleMapPath.remove(mGoogleMapPath.length()-1, 1);

    m_haituProfile = new MapPosProfile("mappos_haitu.ini");
    m_rongheProfile = new MapPosProfile("mappos_ronghe.ini");
}

zchxMapLoadThread::~zchxMapLoadThread()
{
    if (m_haituProfile != NULL)
    {
        delete m_haituProfile;
        m_haituProfile = NULL;
    }

    if (m_rongheProfile != NULL)
    {
        delete m_rongheProfile;
        m_rongheProfile = NULL;
    }
}
...
//    if (task.mSource == ZCHX::TILE_TMS_LOCAL_IMG)
//    {
//        MapLoadSetting rongheTask;
//        memcpy(&rongheTask, &task, sizeof(MapLoadSetting));
//        rongheTask.mSource = ZCHX::TILE_TMS_RONGHE;
//        mTaskList.append(rongheTask);
//    }
}
...
//        qDebug()<<"view_bounds:"<<FLOAT_STRING(view_bounds.min_x, 6)<<FLOAT_STRING(view_bounds.min_y, 6)<<FLOAT_STRING(view_bounds.max_x, 6)<<FLOAT_STRING(view_bounds.max_y, 6);
        ...
        for(int i = tile_start_x; i <= tile_end_x; i++){
            for(int j = 0; j < y_num; j++){
                ...
                ZCHX::Data::Mercator imageMercator(0, 0);
                imageMercator.mX = total_bounds.min_x + (i + 0.5) * MAP_IMG_SIZE * resolution;
                imageMercator.mY = total_bounds.min_y + (tile_start_y + y_num - 1 - j) * MAP_IMG_SIZE * resolution;
                ZCHX::Data::LatLon imageLatLon = zchxMapDataUtils::mercatorToWgs84LonLat(imageMercator);

                //google
                QString url;
                if(task.mSource == ZCHX::TILE_TMS_LOCAL_MAP)
                {
#if 0
                    QString mapPos = m_haituProfile->getMapPos(imageLatLon.lon, imageLatLon.lat);
                    if (mapPos.isEmpty())
                    {
                        continue;
                    }
                    url = QString("%1/%2/%3/%4/%5/%6.png").arg(mImgUrl).arg(mMapFilePath).arg(mapPos).arg(task.mZoom).arg(i).arg(k);
#else
                    url = QString("%1/%2/%3/%4.png").arg(mMapFilePath).arg(task.mZoom).arg(i).arg(k);
#endif

//                    qDebug() << "XXXXXXXXXXXXXXXXXXXXXX" << tile_start_x << tile_end_x << y_num << task.mZoom << imageLatLon.lon << imageLatLon.lat;
//                    qDebug() << "ZZZZZZZZZZZZZZZZZZZZZZ" << url;

                }
                else if(task.mSource == ZCHX::TILE_TMS_LOCAL_IMG)
                {
                    QString mapPos = m_rongheProfile->getMapPos(imageLatLon.lon, imageLatLon.lat);
                    if (mapPos.isEmpty())
                    {
                        continue;
                    }
                    url = QString("%1/%2/%3/%4/%5/%6.png").arg(mImgUrl).arg(mGoogleMapPath).arg(mapPos).arg(task.mZoom).arg(i).arg(k);
                }
                else if(task.mSource == ZCHX::TILE_TMS_RONGHE)
                {
                    QString mapPos = m_rongheProfile->getMapPos(imageLatLon.lon, imageLatLon.lat);
                    if (mapPos.isEmpty())
                    {
                    ...
                    url = QString("%1/%2/%3/%4/%5/%6.png").arg(mImgUrl).arg(mImgFilePath).arg(mapPos).arg(task.mZoom).arg(i).arg(k);
                }
                else if(task.mSource == ZCHX::TILE_GOOGLE_ONLINE)
                {
        ...
        {
            emit signalSendImgList(mTileImgList, task.mMapRange);
        }
    }
}
