...
zchxAisDataProcessor::zchxAisDataProcessor(const zchxAisDataProcessParam& param, QObject *parent) :
    ...
    mChartWorker(0)
{
    PROFILES_INSTANCE->setDefault("Ais", "UseSystemTime", "true");
    mUseSystemTime = PROFILES_INSTANCE->value("Ais", "UseSystemTime").toBool();
    mPrint = false;
    initChartWorker();
...
//        }
        if(mUseSystemTime)
        {
            ...
            if(objObject.contains("UTC"))
            {
                objObject.remove("UTC");
                objObject.insert("UTC", now);
            } else if(objObject.contains("utc"))
            {
                objObject.remove("utc");
                objObject.insert("utc", now);
            }
