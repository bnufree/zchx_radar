...
struct RadarSimpleData
{
    ...
    double        wgs84PosLat;              // WGS84坐标计算纬度
    double        wgs84PosLong;             // WGS84坐标计算经度
    float         cog;                      // 方位角
    float         sog;                      // 速度
    QList<RadarSimpleData*> historyData;
};
...
class ZCHXRadarDataServer : public QObject
{
    Q_OBJECT
public:
    explicit ZCHXRadarDataServer(int source, QObject *parent = 0);
    ~ZCHXRadarDataServer();
    int   sourceID() const;
