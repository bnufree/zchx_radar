...
void zchxRadarTargetTrack::processWithTargetNode(const zchxRadarTrackTask &task)
{
    if(task.size() == 0) return;
    zchxTimeElapsedCounter counter(__FUNCTION__);
    ...
    {
        //根据已经存在的目标查找可能的路径
        TargetNodeMap::iterator it = mTargetNodeMap.begin();
        for(; it != mTargetNodeMap.end(); it++)
        {
            TargetNode *node = it.value();
            int old_src_size = temp_list.size();
            //先更新目标的分支
            for(int i=0; i<node->children.size(); i++)
            {
                TargetNode *child = node->children.at(i);
                if(!child) continue;
                //获取分支的最后节点
                TargetNode *last_child = child->getLastChild(child);
                if(!last_child) continue;
                //根据分支节点的速度和角度取得待更新的矩形单元
                zchxRadarRectDefList result = getDirDeterminTargets(temp_list, last_child->rect, true);
                if(result.size() == 0) continue;
                //更新对应的速度和方向
                zchxRadarRectDef total = result.first();
                QGeoCoordinate last_pos(last_child->rect->centerlatitude(), last_child->rect->centerlongitude());
                QGeoCoordinate cur_pos(total.centerlatitude(), total.centerlongitude());
                double cog = last_pos.azimuthTo(cur_pos);
                double delta_time = list_time - last_child->rect->timeofday();
                if(delta_time <= 0) delta_time += (3600 * 24);
                double cal_dis = last_pos.distanceTo(cur_pos);
                double sog = cal_dis / delta_time;
                total.set_sog(sog);
                total.set_cog(cog);
                TargetNode *new_node= new TargetNode(total);
                last_child->children.append(new_node);
            }
            if(temp_list.size() != old_src_size)
            {
                //分支节点已经更新,结束处理
                continue;
            }
            //更新根节点
            zchxRadarRectDefList result = getDirDeterminTargets(temp_list, node->rect, false);
            for(int i=0; i<result.size(); i++)
            {
                zchxRadarRectDef total = result[i];
                total.set_realdata(true);
                total.set_rectnumber(node->rect->rectnumber());
                QGeoCoordinate last_pos(node->rect->centerlatitude(), node->rect->centerlongitude());
                QGeoCoordinate cur_pos(total.centerlatitude(), total.centerlongitude());
                double cog = last_pos.azimuthTo(cur_pos);
                double delta_time = list_time - node->rect->timeofday();
                if(delta_time <= 0) delta_time += (3600 * 24);
                double cal_dis = last_pos.distanceTo(cur_pos);
                double sog = cal_dis / delta_time;
                total.set_sog(sog);
        ...
        //检查所有目标路径点的个数。若路径存在3个点，则目标航迹确定为对应路径。
        //其他剩余的路径转换为其他目标的点迹，等待下一个周期到来时进行继续更新。航迹目标添加到输出目标队列。
        it = mTargetNodeMap.begin();
        for(; it != mTargetNodeMap.end(); it++)
        {
            TargetNode *node = it.value();
            //先更新目标的分支
            ...
            {
                //取得路径的第一个节点
                TargetNode *child_lvl1 = node->children.at(i);
                if(!child_lvl1) continue;
                //取得路径的第二个节点
                if(child_lvl1->children.size() == 0) continue;
                //第二个节点存在
                TargetNode *child_lvl2 = child_lvl1->children[0];
                //将目标添加到输出队列
                zchxRadarRect result;
                result.set_dirconfirmed(true);
                result.set_estcount(0);
                result.mutable_currentrect()->CopyFrom(*(child_lvl2->rect));
                zchxRadarRectDef *his_rect = result.add_historyrects();
                his_rect->CopyFrom(*(child_lvl1->rect));
                his_rect = result.add_historyrects();
                his_rect->CopyFrom(*(node->rect));
                //从路径列表中删除,这里需要验证是否自动删除了指针
    ...
    for(int i=0; i<temp_list.size(); i++)
    {
        ...
        node->rect->set_rectnumber(mRectNum);
        mTargetNodeMap.insert(mRectNum, node);
