...
enum TargetStatus{
    Target_Status_UnDef = 0,
    Target_Status_Moving,
    Target_Status_Lost,
};

struct Node{
    zchxRadarRectDef                        *mDefRect;
    QList<QSharedPointer<Node>>             mChildren;           //孩子
    Node*                                   mParent;             //父亲
...
public:
    int                                         mSerialNum;
    TargetStatus                                mStatus;
    int                                         mUpdateTime;        //目标最新的更新时间
    QList<int>                                  mVideoIndexList;    //目标对应的回波周期序列
    Node*                                       mTopNode;           //目标对应的开始节点
    Node*                                       mLatestNode;        //目标对应最新的节点。可以通过parent遍历整条路径
...
private:
    ...
    QMap<int, QList<int>>       mTargetCycleIndexMap;               //每一个目标对应的回波周期队列
