    ...
    if(areaTableList.size() > 0)
    {
        ...
        for(; it != counter_map.end(); it++)
        {
            ...
            if(targetList.size() > 0)
            {
                ...
                if(node->mParent)
                {
                    qDebug()<<"node: "<<node->mSerialNum<<" is a child. find the closest rect";
                ...
                } else
                {
                    ...
                    //这里就不进行任何其他操作
                    qDebug()<<"node: "<<node->mSerialNum<<" is a top node. all coresspond rect appended";
                }
