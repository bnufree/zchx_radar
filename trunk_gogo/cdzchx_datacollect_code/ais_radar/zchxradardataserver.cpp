...
ZCHXRadarDataServer::ZCHXRadarDataServer(int source, QObject *parent)
    : QObject(parent),
      mRadarPowerStatus(0),
      m_sReportIP(""),
      m_uReportPort(0),
...

{
    ...
    cout<<"ZCHXRadarDataServer thread id :"<<QThread::currentThreadId();
...
void ZCHXRadarDataServer::updateVideoUdpProgress(const QByteArray& data)
{
    qint64 utc = QDateTime::currentMSecsSinceEpoch();
...
bool ZCHXRadarDataServer::getControlValueRange(INFOTYPE type, int &min, int &max)
{
...
bool ZCHXRadarDataServer::getControlAutoAvailable(INFOTYPE type)
{
    if(!mRadarStatusMap.contains(type)) return false;
    RadarStatus sts = mRadarStatusMap[type];
...
void ZCHXRadarDataServer::ProcessReport(const QByteArray& bytes, int len)
{
...
void ZCHXRadarDataServer::prtVideoSlot(bool a)//打印回波数据
{
    prt = a;
}

void ZCHXRadarDataServer::new_client()
{
    cout<<"新客户段连接";
...
void ZCHXRadarDataServer::client_dis()
{
...
void ZCHXRadarDataServer::read_client_data()
{
    QString str_radar = QString("Radar_%1").arg(m_uSourceID);
...
void ZCHXRadarDataServer::setPixSlot(QPixmap pix)
{
    cout<<"setPixSlot";
