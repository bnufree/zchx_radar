...
Centroid::PopClosedTargetPosition()
{
    if (m_useOriginalTarget) {
        ...
        qDebug()<<__FUNCTION__<<__LINE__<<peakOfTarget->GetSize().RangeExtent();
...
bool
Centroid::PopClosedTargetPositionWithCheck(TargetPosition& pos)
{
    if (m_useOriginalTarget) {
        m_useOriginalTarget = false;
        if(m_mask->GetSize().RangeExtent() < 2) return false;
        pos = m_mask->GetSize().Center();
        return true;
    } else {
        SegmentedTargetImagePtr peakOfTarget = m_is.PopClosedTarget();
//        if(peakOfTarget->GetSize().RangeExtent() < 2) return false;
        pos = peakOfTarget->GetSize().Center();
        pos.range += m_mask->GetSize().minRange;
        return true;
    }
}
