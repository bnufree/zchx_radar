...
public:
    ...
    TargetSize() : maxRangeValid(false), minRangeValid(false), maxAzValid(false), minAzValid(false), priValid(false), debug(false) {}
    ...
    char CompareSize(TargetSize cmp)
    {
        ...
        if(debug)qDebug()<<"this range_extent:"<<myrange<<" cmp_rangemax:"<<cmp.maxRange<<"az_extent:"<<myaz<<" cmp_rangemax:"<<cmp.maxAz;;
        if (cmp.maxRangeValid && (cmp.maxRange < myrange))
        {
            return 1;
        }
        if (cmp.minRangeValid && (cmp.minRange > myrange)) return -1;
        if (cmp.maxAzValid && (cmp.maxAz < myaz))
        {
            return 1;
        }
    ...
    AZIMUTH AzmExtent() const
    {
        return AZIMUTH_EXTENT(minAz, maxAz);
    }
};
