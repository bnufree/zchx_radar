...
#ifndef ZCHX_RADAR_MESSAGES_GGUID_H // -*- C++ -*-
#define ZCHX_RADAR_MESSAGES_GGUID_H
...
/** Definition of a globally-unique message ID. When a message is first created, it is assigned a GGUID value
...
public:
    ...
    GGUID(const std::string& producer, const MetaTypeInfo& typeInfo);

    GGUID(const std::string& producer, const MetaTypeInfo& typeInfo, MetaTypeInfo::SequenceType sequenceNumber);
...
private:
    ...
    GGUID(const GGUID& rhs);

    GGUID& operator=(const GGUID& rhs);
