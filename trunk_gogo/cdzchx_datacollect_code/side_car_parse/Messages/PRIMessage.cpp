...
void
PRIMessage::loadArray( const VideoFrame &frame, uint32_t &count )
{
    Super::load( frame );
    this->riuInfo_.timeStamp = frame.timeofday();
    this->riuInfo_.sequenceCounter = frame.msgindex();
    //        riuInfo.shaftEncoding = static_cast<uint32_t>(azimuth / (2 * M_PI) * radarConfig_->GetShaftEncodingMax());
    this->riuInfo_.shaftEncoding = frame.azimuth();
    this->riuInfo_.prfEncoding = 0;
    this->riuInfo_.deviceTime = 0.0;
    this->riuInfo_.rangeMin = frame.startrange();
    this->riuInfo_.rangeFactor = frame.rangefactor() / 1000.0; //米换算公里
    count = frame.amplitude_size();
    //        qCDebug( radarmsg ) << "azimuth" << riuInfo.shaftEncoding;
}
...
void
Traits::Int16::Reader( const VideoFrame& frame, size_t size, std::vector<Type> &data )
{
    if ( size )
    {
        for ( int i = 0; i < frame.amplitude_size(); i++ )
        {
            data.push_back( frame.amplitude( i ) );
        }
    }
}
