...
RadarConfig::RadarConfig( QObject *parent )
    ...
    , mHead(180.0)
    , mParseMode(1)
    , mExtractionMaxBin(7000) //range/ factor (7000 / 13.5)
    , mExtractionMaxAmz(0.3)
    , mDiscardExtractionMaxBin(120000)
    , mDiscardExtractionMaxAmz(M_PI)
    , mCorratedRadius(5)
    , mCorratedScansNum(2)
    , mTargetSearchRadius(10)
    , mTargetMaxInitRotationCount(2)
    , mTargetMaxCoastRotationCount(5)
    , mMergeRadius(100)
    , mMergeAzimuth(10)
{
}
...
//    mRangeFactor = (mRangeMax - mRangeMin) / (mGateCountMax - 1);
    mRangeFactor = (mRangeMax - mRangeMin) / mGateCountMax;
    return mRangeFactor;
}
