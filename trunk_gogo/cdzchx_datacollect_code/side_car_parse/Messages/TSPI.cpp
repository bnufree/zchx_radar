...
TrackPoint TSPI::toTrackPoint() const
{
    ...
    trackPoint.set_wgs84poslong(GeoIns->geoRads2DD(getLongitude()));
    trackPoint.set_wgs84poslat(GeoIns->geoRads2DD(getLatitude()));
    trackPoint.set_tracklastreport(false);
    trackPoint.set_cartesiantrkvel_vx(0);
    trackPoint.set_cartesiantrkvel_vy(0);
    trackPoint.set_cog(ZchxRadarUtils::radiansToDegrees(getAzimuth()));
